...
void MakeTicks(game_manager::GameSession& session, uint64_t dur, size_t number) {
    ...
    for (int i = 0; i < number; i++) {
        session.Tick(dur, [](std::vector<game_manager::Retiree>&&){});
    }
}
...
bool CheckOneObjectOneRoad(const game_manager::LootObject& obj, const model::Road& road) {
    ...
    return l <= obj.position.x && obj.position.x <= r
            && t <= obj.position.y && obj.position.y <= b;
...
TEST_CASE("model_tests") {
    ...
    game_manager::GameSession session{ioc, data.map, *data.move_map, data.loot_config, true, 10.};
