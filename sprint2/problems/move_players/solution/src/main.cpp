...
int main(int argc, const char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: game_server <game-config-json> <static-data-path>"sv << std::endl;
        return EXIT_FAILURE;
    }
    try {
        ...
        const std::filesystem::path home_path(std::filesystem::current_path().parent_path().parent_path());
        const std::filesystem::path static_path(std::filesystem::weakly_canonical(home_path / argv[2]));

        game_manager::GameManager game_m{game, ioc};
