...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (FindAndCutTarget(req_info, "/api/v1/game"sv)) {
            ...
            } else if (FindAndCutTarget(req_info, "/action"sv)) {
                return MoveResponse(req, req_info, send);
            } else {
...
private:
                         ...
                         RequestInfo& req_info, Send&& send)
    {
        ...
        if (!jv.as_object().contains("move"s)) {
            return send(json_resp::MakeBadRequestResponse(req, "Failed to parse action", "invalidArgument", true));
        }

        std::optional<game_manager::Direction> dir = game_manager::GetDirectionFromString(jv.at("move"s).as_string());
                ...
                if (res == Result::ok) {
                    ...
                    for (const Player& player : *players) {
                        ...
                        json::value jv = {
                            ...
                            {"dir", game_manager::GetStringDirection(state.dir)}
                        };
