...
void tag_invoke (json::value_from_tag, json::value& jv, const PlayerInfo& pi) {
    jv = {
        {json_keys::auth_token_key, pi.token},
        {json_keys::player_id_key, pi.Id}
    };
}
} // game_manager
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (FindAndCutTarget(req_info, "/api/v1/game"sv)) {
            if (FindAndCutTarget(req_info, "/join"sv)) {
                ...
                return PlayersResponse(req, req_info, send);
            } else {
...
private:
    ...
    template <typename Body, typename Allocator, typename Send>
    void PlayersResponse(http::request<Body, http::basic_fields<Allocator>>& req, RequestInfo& req_info, Send&& send) {
