...
LootType tag_invoke(json::value_to_tag<LootType>&, const json::value type) {
    ...
    res.value = type.at(json_keys::value_key).as_int64();
...
Map tag_invoke(json::value_to_tag<Map>&, const json::value map) {
    ...
    if (map.as_object().contains(map_dog_speed_key)) {
        config.dog_speed = map.as_object().at(map_dog_speed_key).as_double();
    }

    if (map.as_object().contains(map_bag_capacity_key)) {
        config.bag_capacity = map.as_object().at(map_bag_capacity_key).as_uint64();
    }
