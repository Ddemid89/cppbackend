...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    double default_speed = model::DEFAULT_DOG_SPEED;

    if (config_json.as_object().contains(json_keys::game_def_speed_key)) {
        default_speed = config_json.as_object().at(json_keys::game_def_speed_key).as_double();
    }

    model::LootConfig loot_config = json::value_to<model::LootConfig>(config_json.at(json_keys::loot_config_key));

    model::Game game(default_speed, loot_config);
